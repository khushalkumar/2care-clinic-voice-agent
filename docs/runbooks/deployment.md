...
2. Confirm the AWS account is fully verified and the selected deployment identity can use ECR, ECS, RDS, ALB, Secrets Manager, IAM, CloudWatch, and ACM. Before production voice traffic, issue and validate an ACM certificate and pass its ARN as `certificate_arn`; the ALB then redirects HTTP to HTTPS and advertises an HTTPS tool base URL to Retell.
